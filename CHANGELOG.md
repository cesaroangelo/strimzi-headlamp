...
- `THIRD-PARTY-LICENSES.md`, generated by `npm run licenses` and shipped inside
  every release archive, covering the dependencies bundled into `dist/main.js`
...
- Dependencies refreshed: `@xyflow/react` to 12.11.6, `typescript-eslint` to 8,
  `eslint-plugin-react-hooks` to 5, plus Prettier, React and Node type updates
- CI now tests on Node 20 and 22, and builds on Node 22; Node 18 is end of life
- Documentation rewritten and shortened. Local testing and release steps moved
  out of the README into `docs/LOCAL-TESTING.md` and `docs/RELEASING.md`
...
### Fixed (tooling)
- `npm run format` failed because the `prettier` key in `package.json` pointed
  at a package that is not installed. That key and the equally dead
  `eslintConfig` key are gone; `.prettierrc.json` and `.eslintrc.json` are now
  the only configuration, and the source is formatted to match
